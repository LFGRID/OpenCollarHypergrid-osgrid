    ...
    if (iFound)
    {
        ...
        if (sToken=="owner") 
            g_lOwners = lPeople;
             ...
             g_lTempOwners = lPeople;
        else if (sToken=="secowner") 
            g_lSecOwners = lPeople;
        else if (sToken=="blacklist") 
            g_lBlackList = lPeople;
...
{ // here iNum: auth value, sStr: user command, kID: avatar id  
    ...
    { 
        if (iNum == COMMAND_OWNER || kID == g_kWearer) 
        {
            ...
            Notify(kID, "Only Owners & Wearer may access this command",FALSE);
        if (remenu) 
            AuthMenu(kID, iNum);
    } 
    ...
    { //add a person to a list
        ...
        list params1 = llParseString2List (sTmpName, ["-"], []);//this is because if((key)name) does not work in IW
        string kNewOwner1 = llList2String(params1, 4);//Ditto here
        if (iNum!=COMMAND_OWNER) 
        {
            Notify(kID, sOwnerError, FALSE);
            if (remenu) AuthMenu(kID, Auth(kID,FALSE));
        }
        else if (kNewOwner1 !="")//this result should not be blank if a true key
//        else if ((key)sTmpName)//NG - else if ((key)sTmpName) don't work in IW
        {
...
default
{
    ...
    {  
        ...
        { //authenticate messages on COMMAND_NOAUTH
            ...
            else if (kID != g_kWearer && iAuth == COMMAND_OWNER && sStr == "runaway") 
            {  //owner requests the runaway menu
        ...
        else if (iNum == LM_SETTING_RESPONSE) 
        {
            ...
            if (llGetSubString(sToken, 0, i) == g_sScript) {
                ...
                else if (sToken == "tempowner") 
                {
                    ...
                    // only say the owner list if it has changed (including on_rez)
                    if (llGetListLength(g_lTempOwners) && tmpowners != g_lTempOwners) 
                        SayOwners();
                } 
                ...
                else if (sToken == "runawaydisable") 
                    g_iRunawayDisable = (integer)sValue;
        ...
        else if (iNum == LM_SETTING_EMPTY) 
        {
            ...
            if (llGetSubString(sStr, 0, i) == g_sScript) 
            {
                ...
                else if (sStr == "groupname") 
                    g_sGroupName = "";
                else if (sStr == "openaccess") 
                    g_iOpenAccess = FALSE;
                else if (sStr == "limitrange") 
                    g_iLimitRange = TRUE;
                else if (sStr == "runawaydisable") 
                    g_iRunawayDisable = FALSE;
                else if (sStr == "secowners") 
                    g_lSecOwners = [];
                else if (sStr == "blacklist") 
                    g_lBlackList = [];
            }
        } 
    ...
    // JS: Remove ATTACHMENT_REQUEST & RESPONSE after all attachments have been updated properly
        ...
        else if (iNum == DIALOG_RESPONSE) 
        {
            ...
            if (~iMenuIndex) 
            {
                ...
                {   //no chat commands for this menu, by design, so handle it all here
                    if (sMessage == UPMENU) 
                    {
                        AuthMenu(kAv, iAuth);
                    } 
        ...
        { //reply from add-by-name or add-from-menu (via FetchAvi dialog)
            if (kID == REQUEST_KEY) 
            {
                ...
                if (llList2String(params, 0) == g_sScript) 
                {
                    ...
                    //break key apart to check correct formatting
                     //if last bit is not blank then it should be a normal key format
                     //In SL we just call  -if ((key)kNewOwner) to check if valid key
                    list params1 = llParseString2List(llList2String(params, 5), ["-"], []);
                    string kNewOwner1 = llList2String(params1, 4);
                    if (kNewOwner1 !="") 
                    {
    ...
    { //response to a group name lookup
